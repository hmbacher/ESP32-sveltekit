		...
		>
			...
			<div class="grid w-full grid-cols-1 content-center gap-x-4 gap-y-2 px-4">
				...
				<div>
					...
					<input
						type="text"
						class="input w-full invalid:border-error invalid:border-2 {formErrors.discovery_prefix
				...
				<div>
					...
					<input
						type="text"
						class="input w-full {formErrors.device_name ? 'border-error border-2' : ''}"
					...
					{#if formErrors.device_name}
						<div transition:slide|local={{ duration: 300, easing: cubicOut }}>
							<label class="label" for="device_name">
								<span class="text-error">Device name is limited to 64 characters</span>
							</label>
						</div>
					{/if}
				</div>
				...
				<div>
					...
					<input
						type="text"
						class="input w-full {formErrors.manufacturer ? 'border-error border-2' : ''}"
					...
					{#if formErrors.manufacturer}
						<div transition:slide|local={{ duration: 300, easing: cubicOut }}>
							<label class="label" for="manufacturer">
								<span class="text-error">Manufacturer is limited to 64 characters</span>
							</label>
						</div>
					{/if}
				</div>
				...
				<div>
					...
					<input
						type="text"
						class="input w-full {formErrors.model ? 'border-error border-2' : ''}"
			...
			<div class="mx-4 flex flex-wrap justify-end gap-2">
				<button
					...
					type="submit">Apply Settings</button
				>
			</div>
		</form>
	</Collapsible>
